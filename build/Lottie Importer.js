...
function extractGradientInfo(gfItem) {
  ...
  return { type, startPt, endPt, stops };
}
...
function keyframeAnimatedTransforms(nodeId, ks, yFlip, hasParent, compW, compH, scaleFactor, timeOffset, precompDims) {
  ...
  if (ks.s && ks.s.a === 1 && ks.s.k && ks.s.k.length > 1) {
    var scaleXKfs = [];
    var scaleYKfs = [];
    var needsAnchorComp = !posAnimated && (Math.abs(ax) > 1e-3 || Math.abs(ay) > 1e-3);
    var compPosXKfs = [];
    var compPosYKfs = [];
...
function importLayerSet(layers, assets, yFlip, scaleFactor, compW, compH, groupId, timeOffset, precompCache, frameRate) {
  ...
  for (var j = 0; j < layers.length; j++) {
    ...
    if (childId && parentId && childId !== parentId) {
      try {
        ...
        console.log("Lottie Importer: Could not parent '" + (l.nm || l.ind) + "': " + e.message);
      }
    }
  }
  if (groupId) {
  ...
  for (var tm = 0; tm < layers.length; tm++) {
    ...
    try {
      api.set(matteId, { "hidden": true });
    } catch (e) {
      console.log("Lottie Importer: Could not hide matte " + matteId + ": " + e.message);
    }
